...
//
// Abstract C++ interface for NitroWebgpuPresent.
// Mirrors RN Nitro's HybridXxxSpec pattern — pure-virtual C++ with stdlib types.
// 1. Subclass HybridNitroWebgpuPresent and implement all pure-virtual methods.
// 2. Register via nitro_webgpu_present_register_factory() (multi-instance) or
//    nitro_webgpu_present_register_impl() (single-instance legacy API).
#pragma once
#ifndef NITRO_WEBGPU_PRESENT_NATIVE_G_H
#define NITRO_WEBGPU_PRESENT_NATIVE_G_H
...
/// 2. Call nitro_webgpu_present_register_impl(&myInstance) at startup.
/// 3. Call nitro_webgpu_present_register_impl(nullptr) in teardown.
class HybridNitroWebgpuPresent {
public:
    virtual ~HybridNitroWebgpuPresent() = default;

    // ── Methods ──────────────────────────────────────────────────────────
    // source: nitro_webgpu_present.native.dart:27
    virtual int64_t createPresenter(int64_t deviceAddress, int64_t widthPx, int64_t heightPx) = 0;
...
protected:
    HybridNitroWebgpuPresent() = default;
};
...
using HybridNitroWebgpuPresentFactory = std::function<std::shared_ptr<HybridNitroWebgpuPresent>(const std::string&)>;
#endif
...
void nitro_webgpu_present_register_factory(void* factory_fn_ptr);

/// Register a single raw-pointer implementation (legacy single-instance API).
/// Use register_factory() instead for new code.
void nitro_webgpu_present_register_impl(HybridNitroWebgpuPresent* impl);

/// Return the registered raw-pointer impl, or nullptr (legacy API).
HybridNitroWebgpuPresent* nitro_webgpu_present_get_impl(void);

#ifdef __cplusplus
} // end extern "C"

/// C++-only typed factory registration — preferred over the C void* variant.
inline void nitro_webgpu_present_register_factory_typed(HybridNitroWebgpuPresentFactory factory) {
    nitro_webgpu_present_register_factory(static_cast<void*>(&factory));
}
#endif

#endif // NITRO_WEBGPU_PRESENT_NATIVE_G_H
