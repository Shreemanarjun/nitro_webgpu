// HybridNitroWebgpu — NativeImpl.cpp implementation.
// Generated by nitrogen link. Fill in the method bodies.
#include "../lib/src/generated/cpp/nitro_webgpu.native.g.h"

#include <string>

class HybridNitroWebgpuImpl final : public HybridNitroWebgpu {
public:
    // TODO: implement all pure-virtual methods declared in HybridNitroWebgpu
};

static HybridNitroWebgpuImpl g_impl;

// Auto-register on shared library load — no manual init call needed.
#if defined(_WIN32) || (defined(__linux__) && !defined(__ANDROID__))
#if defined(_WIN32)
namespace {
  struct _AutoRegister {
    _AutoRegister() { nitro_webgpu_register_impl(&g_impl); }
  };
  _AutoRegister _auto_register_instance;
}
#else
__attribute__((constructor))
static void nitro_webgpu_auto_register() {
    nitro_webgpu_register_impl(&g_impl);
}
#endif
#endif // auto-register on C++ platforms
